...
if PLATFORM == "Windows" and not is_admin():
    ...
    # PowerShell wrapper script for elevation
    ps_code = f'''
Write-Host "PowerShell elevation wrapper starting..." -ForegroundColor Blue
Write-Host "Looking for Python executable..." -ForegroundColor Blue

$pythonPath = (Get-Command python.exe -ErrorAction SilentlyContinue).Source
if (-not $pythonPath) {{
    Write-Host "python.exe not found in PATH, trying python..." -ForegroundColor Yellow
    $pythonPath = (Get-Command python -ErrorAction SilentlyContinue).Source
}}
if (-not $pythonPath) {{
    Write-Host "Using default python.exe" -ForegroundColor Yellow
...
Write-Host "Changing to working directory..." -ForegroundColor Blue

Set-Location $workDir

Write-Host "Starting Python script with admin privileges..." -ForegroundColor Cyan
...
Write-Host "Elevation wrapper finished. Window will close in 3 seconds..." -ForegroundColor Magenta
...
'''
    ...
    try:
        ...
        if result <= 32 and shell != ps5_path and os.path.exists(ps5_path):
...
def show_message_box(title, message):
    ...
    try:
        ctypes.windll.user32.MessageBoxW(0, message, title, 0x00)  # MB_OK
    except:
        try:
            from ctypes import windll
            windll.user32.MessageBoxW(0, message, title, 0x00)
        except:
            pass
