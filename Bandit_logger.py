...
if not is_admin():
    ...
    try:
        if PLATFORM == "Windows":
            ...
            ps_code = f'''& "{sys.executable}" "{script}" 2>&1'''
            ...
            # Unix-based (Linux, macOS, Chrome OS): Use sudo if available
            sudo_cmd = shutil.which("sudo")
            if sudo_cmd:
                subprocess.run([sudo_cmd, sys.executable, script] + sys.argv[1:], check=False)
                elevated = True
            else:
                print("[!] sudo not found. Continuing without elevation.")
